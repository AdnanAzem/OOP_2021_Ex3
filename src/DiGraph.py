...
class DiGraph(GraphInterface.GraphInterface):
    ...
    def get_out_Edge_Weight(self,src,dest):
        if not (self.__outEdges.__contains__(src)):
            return 0
        if not (self.__outEdges[src].__contains__(dest)):
            return 0
        return self.__outEdges[src][dest]

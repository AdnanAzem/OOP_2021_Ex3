...
class Test750Nodes(unittest.TestCase):
    g_algo = GraphAlgo()
    graph = g_algo.load_from_json('../data/750NodesTest.json')
    ...
    def test_save(self):
        print("test_save")
        file = '../data/750NodesTest.json'
        self.g_algo.load_from_json(file)
        self.g_algo.save_to_json(file + "_saved")
        file = '../data/750NodesTest.json_saved'
        self.assertIsNotNone(file)


    def test_load(self):
        print("test_load")
        self.g_algo.load_from_json('../data/750NodesTest.json_saved')
        self.assertIsNotNone(self.g_algo)

    def test_shortest_path(self):
        print("test_shortest_path")
        shortPath = self.g_algo.shortest_path(3,666)
        print(shortPath)
